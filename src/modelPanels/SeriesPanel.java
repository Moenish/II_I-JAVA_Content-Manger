...
public class SeriesPanel extends JPanel {
    ...
    public SeriesPanel(Series series) {
        ...
        if (series.getImgLink() != null) {
            try {
                image = ImageIO.read(new File(series.getImgLink()));
            } catch (IOException e) {
                try {
        ...
        JLabel episodesTotalLabel = new JLabel("<html><font color='white'>Total Episodes: </font></html>");
        JLabel episodesWatchedLabel = new JLabel("<html><font color='white'>Watched Episodes: </font></html>");
        JLabel seasonsTotalLabel = new JLabel("<html><font color='white'>Total Seasons: </font></html>");
        JLabel seasonsWatchedLabel = new JLabel("<html><font color='white'>Watched Seasons: </font></html>");
        ...
        c1.add(episodesTotalLabel);   c1.add(fields[7]);
        c1.add(episodesWatchedLabel); c1.add(fields[8]);
        c1.add(seasonsTotalLabel);    c1.add(fields[9]);
        c1.add(seasonsWatchedLabel);  c1.add(fields[10]);
    ...
    private void setFields() {
        for (int i = 0; i < 15; i++) {
            ...
            String text = "";
            if (i == 0) text = series.getTitle();
            if (i == 1) text = series.getPublisher();
            if (i == 2) text = series.getWikiLink();
            if (i == 3) text = series.getImgLink();
            if (i == 4) text = series.getCategory();
            if (i == 5) text = series.getSubCategory();
            if (i == 6) text = series.getDirector();

            if (i == 7) text = String.valueOf(series.getEpisodesTotal());
            if (i == 8) text = String.valueOf(series.getEpisodesWatched());
            if (i == 9) text = String.valueOf(series.getSeasonsTotal());
            if (i == 10) text = String.valueOf(series.getSeasonsWatched());

            if (i == 11) text = String.valueOf(series.getRatings().getRatingMax());
            if (i == 12) text = String.valueOf(series.getRatings().getRatingCrt());
            if (i == 13) text = String.valueOf(series.getRatings().getRatingPrs());
            if (i == 14) text = String.valueOf(series.getRatings().getRatingNbr());
